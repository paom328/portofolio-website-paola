...
switch(figure){
    case 'triangle':
        ...
        window.alert(`El área del triángulo es ${(base*height)/2}`);
        break
    case 'rectangle':
        base=prompt('Introduce la base del rectángulo')
        height=prompt('Introduce la altura del rectángulo')
        window.alert(`El área del rectángulo es ${base*height}`);
        break
    case 'circle':
        radius=prompt('Introduce el radio del círculo')
        window.alert(`El área del círculo es ${Math.PI * Math.pow(radius,2)}`);
        break
    default: window.alert('La figura geométrica no es válida');
}